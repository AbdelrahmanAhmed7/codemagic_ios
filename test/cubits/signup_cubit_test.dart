...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(RegisterRequestModel(
      cardNo: '', nationalId: '', phoneNumber: '', password: '', confirmPassword: '',
    ));
    const channel = MethodChannel('plugins.it_nomads.com/flutter_secure_storage');
    channel.setMockMethodCallHandler((MethodCall call) async => null);
  });
  ...
  blocTest<SignupCubit, SignupState>(
    ...
    build: () {
      when(() => repo.register(any<RegisterRequestModel>(), any())).thenAnswer((_) async => ApiResult.success(
    ...
    expect: () => [
      ...
      isA<SignupState>().having((s) => s.mapOrNull(success: (v) => v), 'is success', isNotNull),
    ],
  );
  ...
  blocTest<SignupCubit, SignupState>(
    ...
    build: () {
      when(() => repo.register(any<RegisterRequestModel>(), any())).thenAnswer((_) async => const ApiResult.failure('err'));
      return cubit;
    },
    act: (c) => c.signup('c', 'n', 'p', 'pw', 'pw', 'ar'),
    wait: const Duration(milliseconds: 50),
    expect: () => [
      const SignupState.loading(),
      isA<SignupState>().having((s) => s.mapOrNull(failed: (v) => v.error), 'is failed', isNotNull),
    ],
  );
}
